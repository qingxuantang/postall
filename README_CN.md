...
- **🎯 Director 审核系统** - AI 质量控制，输出**逐条可执行的修改建议**（位置 + 原文 + 替换文本，不只是打分）
- **📅 时效性上下文** - 自动检测内容引用是否过时，保证生成内容使用当前的 AI 工具术语
- **📱 多平台发布** - Twitter/X、LinkedIn、Instagram、Pinterest、Threads、小红书
- **🎨 图片生成** - AI 自动生成配图（Gemini，带重试与退避）
- **📊 内容策略** - 定义内容支柱、主题，保持均衡的内容组合
- **📈 RLHF 学习** - 系统从你的反馈中持续改进
- **⏰ 智能排期** - 每个平台的最佳发布时间
- **🔄 守护进程模式** - 持续运行，自动生成和发布
- **🛡️ 生产级安全** - 限速、原子发布锁、崩溃恢复、12 小时滑动窗口 — 防止 daemon 重启后的群发事故
...
- **平台适配** - 是否针对目标平台进行了优化？
- **事实准确性** - 没有编造的统计数据或声明
- **编造证言检测** - 识别虚构的用户故事（具名人物 / 假对话引用）且无原始凭证（截图 / DM 图）时直接 REJECT
- **合规性** - 没有问题内容

#### 逐条可执行的修改反馈

每条扣分项，Director 会输出一个结构化的 `ReviewIssue`，包含：

- `dimension` —— 命中的评分维度
- `deduction` —— 扣了几分
- `location` —— 内容里的位置（例如"第2段"、"开头一句"）
- `original_text` —— 出问题的原文片段
- `suggestion` —— 具体的替换文本
...
这把审核从"打个分"变成"复制粘贴就改完"的编辑工作流。

### 时效性上下文

生成的内容会快速过时——"ChatGPT 4 刚发布" / "OpenAI 最新动态"这种引用很快就站不住。PostAll 的时效性模块：

- 维护一份"当前热门 AI 工具 / 当年趋势 / 已过时的引用"的清单
- 每次生成时，抓 Hacker News 首页，自动识别新工具
- 已知工具自动加入 `current_hot_tools`；未知工具标记给人确认
- 把这份最新清单注入生成 prompt，确保产出内容用的是当前的工具术语

### 生产级安全（守护进程模式）

发布守护进程内置了一组**真上服务器才能体感到价值**的安全机制：

- **限速** —— 每 `PUBLISH_RATE_WINDOW_HOURS` 时间窗内最多发 `PUBLISH_RATE_LIMIT` 篇（默认每 2 小时 3 篇）。daemon 重启后哪怕队列里有一堆 due 的帖子，也不会一次性全发出去（社交平台会判机器行为）
- **12 小时滑动窗口** —— 排程时间超过 12 小时前的帖子视为已弃用，永不自动发布；要重发必须手动重排
- **原子发布锁** —— 调用发布器前，`claim_post_for_publishing` 原子地把状态从 `scheduled → publishing`；并发循环或多个 daemon 实例不会发同一帖两次
- **崩溃恢复** —— 启动时把卡在 `publishing` 状态的行（来自 SIGKILL / OOM 中断）标为 `failed` 而不是 `scheduled`，防止下次重启误再发
- **预过滤未配置平台** —— 不消耗限速配额给你还没配置的平台

每条单独都有用；组合起来才能让 daemon 重启即使队列脏也不会出事。
